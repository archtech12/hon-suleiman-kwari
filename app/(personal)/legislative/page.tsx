  ...
  const policyPositions = [
    {
      ...
      description: "Advocating for reforms that would reduce Nigeria's dependency on imports and make essential goods more affordable for citizens.",
      icon: "trending_down"
    },
    {
      id: 2,
      title: "Infrastructure Development",
      description: "Pushing for improved transportation infrastructure to reduce costs and address fuel scarcity issues affecting communities.",
      icon: "local_shipping"
    },
    {
      id: 3,
      title: "Local Manufacturing Support",
      description: "Promoting domestic production initiatives to reduce reliance on foreign goods and create local employment.",
      icon: "factory"
    },
    {
      id: 4,
      title: "Constituency Advocacy",
      description: "Strong representation for Gaya/Ajingi/Albasu communities with policies that benefit the broader Nigerian population.",
      icon: "groups"
    }
  ]
    ...
    <div className="w-full">
      <section className="bg-green-900/90 dark:bg-green-900/95 py-12 sm:py-16 text-center text-white">
        <div className="max-w-4xl mx-auto px-4 sm:px-6 lg:px-8">
          <h1 className="text-3xl sm:text-4xl lg:text-5xl font-black tracking-tighter">{legislativeData?.title || "Legislative Work"}</h1>
          <p className="mt-4 text-base sm:text-lg text-green-100">Advocating for policies that transform communities and strengthen Nigeria's economy</p>
        </div>
      </section>
      ...
      <section className="py-16 md:py-20 bg-gray-50 dark:bg-gray-900">
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
          <div className="text-center mb-12">
            <h2 className="text-3xl font-bold mb-4 text-gray-900 dark:text-white">Policy Positions</h2>
            ...
            {policyPositions.map((position) => (
              <div key={position.id} className="bg-white dark:bg-gray-800 rounded-xl shadow-lg p-6 hover:shadow-xl transition-shadow">
                <div className="w-12 h-12 bg-green-100 dark:bg-green-900 rounded-full flex items-center justify-center mb-4">
                  <span className="material-symbols-outlined text-green-700 dark:text-green-400">{position.icon}</span>
                </div>
                <h3 className="text-xl font-bold mb-2 text-gray-900 dark:text-white">{position.title}</h3>
      ...
      <section className="py-16 md:py-20 bg-white dark:bg-gray-800">
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
          <div className="text-center mb-12">
            <h2 className="text-3xl font-bold mb-4 text-gray-900 dark:text-white">Recent Legislative Actions</h2>
            <p className="text-gray-600 dark:text-gray-400 max-w-2xl mx-auto">
            ...
            <div className="space-y-6">
              {legislativeData?.bills && legislativeData.bills.length > 0 ? (
                legislativeData.bills.map((bill, index) => (
                  <div key={index} className="bg-gray-50 dark:bg-gray-700 rounded-xl p-6 border-l-4 border-green-700">
                    <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4 mb-3">
                      <h3 className="text-xl font-bold text-gray-900 dark:text-white">{bill.name}</h3>
                      <div className="flex items-center gap-4">
                        <span className="text-sm text-gray-500 dark:text-gray-400">{bill.date || 'N/A'}</span>
                        <span className={`inline-block px-3 py-1 rounded-full text-xs font-medium ${
                          bill.status === 'Proposed' ? 'bg-blue-100 text-blue-800' :
        ...
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
          <div className="bg-white dark:bg-gray-800 rounded-xl shadow-lg p-8 md:p-12">
            <div className="max-w-4xl mx-auto text-center">
              <h2 className="text-3xl font-bold mb-6 text-gray-900 dark:text-white">Economic Reform Vision</h2>
              <div className="prose prose-lg max-w-none text-gray-700 dark:text-gray-300 mx-auto">
                ...
                <div className="bg-green-50 dark:bg-gray-700 rounded-lg p-6 mb-6">
                  <blockquote className="text-lg italic text-gray-900 dark:text-white">
                    "When we make it easier for goods to enter Nigeria, we reduce prices for everyone. This is not just about my constituency, 
                    but about creating prosperity for all Nigerians."
                  </blockquote>
                </div>
                <Link 
                  href="/contact" 
